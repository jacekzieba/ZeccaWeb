...
test("adds, edits and removes an empty portfolio in fake sync", async ({ page }) => {
  ...
  await form.getByRole("combobox", { name: "Typ portfela" }).selectOption("IKE");
  ...
  await expect(form.getByRole("combobox", { name: "Typ portfela" })).toHaveValue("IKE");
  await form.getByPlaceholder("np. IKZE, Obligacje, Interactive Brokers").fill("E2E Portfel edytowany");
  await form.getByRole("combobox", { name: "Typ portfela" }).selectOption("IKZE");
  ...
  await expect(form.getByRole("combobox", { name: "Typ portfela" })).toHaveValue("IKZE");
